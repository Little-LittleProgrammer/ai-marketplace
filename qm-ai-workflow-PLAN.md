...
- 记忆系统：结合 AGENTS.md 和知识库沉淀，持续优化开发效率

### 1.2 核心特性

| 特性 | 描述 |
|------|------|
| **Multi-Agent 协作** | 9 个专业 Agent 协同工作，Router 中心调度 |
| **分层架构** | Agent 层（管理者）+ Skill 层（执行者） |
| **智能路由** | phase-router 通过 LLM 智能意图识别进行路由 |
| **记忆系统** | 全局知识（AGENTS.md）+ 局部知识（context/） |
...
| 阶段 | 输入 | 输出 | 处理内容 | 触发命令 |
|------|------|------|----------|----------|
| **1. 需求分析** | 用户需求文档 + AGENTS.md | spec.md | 需求拆分、边界定义、用户故事 | `/qm-ai:start`（进入 ANALYSIS）；推进用 `/qm-ai:continue` |
...
| **6. 知识沉淀** | 项目经验 | 更新 AGENTS.md | 经验总结、问题记录、模式沉淀、最佳实践 | `/qm-ai:knowledge` 或 `/qm-ai:optimize-flow`（等价） |
...
┌─────────────────────────────────────────────────────────────┐
│                     Commands 层                              │
│  init, start, continue, rollback, status,                  │
│  knowledge（≈optimize-flow）, optimize-flow                  │
├─────────────────────────────────────────────────────────────┤
│                     Agent 层（管理者）                        │
│  负责生命周期管理、调度协调、流程控制                          │
├─────────────────────────────────────────────────────────────┤
│  phase-router │ requirement-manager │ design-manager         │
│  task-decomposer │ code-executor │ test-generator           │
│  frontend-coder │ backend-coder │ experience-depositor      │
└─────────────────────────────────────────────────────────────┘
                           │ 调用
                           ▼
┌─────────────────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────────────────┐
│                     数据存储层                               │
│  AGENTS.md │ requirement/ │ context/ │ workspace/            │
└─────────────────────────────────────────────────────────────┘
```
...
│manager       │ │ manager      │ │ decomposer   │ │ executor     │
└──────┬───────┘ └──────┬───────┘ └──────┬───────┘ └──────┬───────┘
       │                │                │                │
       │                │                │     ┌──────────┴──────────┐
       │                │                │     ▼                     ▼
...
| **agents-memory-maintainer** | AGENTS.md维护 | 项目经验沉淀 | experience-depositor |

#### 服务分析 Skills (4个)
| Skill | 职责 | 触发条件 | 调用者 |
|-------|------|----------|--------|
| **service-overview** | 服务概览 | 分析现有服务 | explore；requirement-manager；/qm-ai:init |
| **service-business** | 业务逻辑分析 | 理解业务流程 | explore；requirement-manager；/qm-ai:init |
...
| Command | 功能 | 参数 |
|---------|------|------|
| **/qm-ai:init** | 冷启动：分析代码库并生成/更新 AGENTS.md、context/ | 无 |
...
### 4.4 Hooks（SessionEnd 组合）

| 类型 | 事件 | 功能 | 触发时机 |
|------|------|------|----------|
| **command** | SessionEnd | 若存在 `.qm-ai/state.json`，用脚本刷新 `updated_at` | 会话结束 |
| **prompt** | SessionEnd | 提醒 requirement/ 产物是否已落盘；建议下一会话 `/qm-ai:status` | 会话结束 |
...
| 当前状态 | 允许的下一状态 | 限制条件 |
|---------|---------------|---------|
| IDLE | ANALYSIS | 无（通常由 `/qm-ai:start`） |
...
| 知识沉淀 | COMPLETE | AGENTS.md更新 | /qm-ai:knowledge 或 /qm-ai:optimize-flow |
...
- SessionEnd：**command** 钩子执行 `scripts/session-end-persist.sh`，在存在 `.qm-ai/state.json` 时刷新 `updated_at`（确定性落盘）
- **prompt** 钩子补充提醒未保存的阶段产物
- 状态文件路径：`.qm-ai/state.json`；阶段推进仍由对话中写入/合并
- 下次会话可通过 `/qm-ai:status` 与 `state.json` 恢复上下文
