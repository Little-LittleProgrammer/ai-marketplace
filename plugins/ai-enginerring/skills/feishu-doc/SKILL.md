...
| `output_path` | 下载保存路径（可选，默认为当前工作目录根目录下的 `${workspace}/specs/文档标题/content.pdf`） | 用户指定 |
...
| `-b, --base-dir` | 基础输出目录 | `${workspace}/specs` |
...
# 6. 解析 PDF 内容（自动保存到 specs/文档标题/ 目录）
...
6. **输出目录结构**：所有解析结果统一保存到 `specs/文档标题/` 目录，包含：
   - `content.pdf` - PDF文件副本
   - `content.md` - 提取的文字内容
   - `images/` - 提取的图片目录
